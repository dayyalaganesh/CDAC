...
Scenario
An AI classification pipeline processes raw data inputs. 
Each raw input is a tuple of string annotations describing a product name, its price, and rating.
The pipeline needs to clean, filter, and sort these records.

Problem Description
Write a function process_dataset(dataset) that processes a dataset using built-in higher-order functions (map, filter) 
...
Mapping: Use map() with a lambda function to transform the filtered entries into dictionaries of the following 
structure: {"product": <name>, "price": <float_price>, "score": <float_rating>}.
...
def process_dataset(dataset):
    parsed_tuples = map(
        lambda t: (
            t[0].strip(), 
            float(re.search(price_pattern, t[1]).group(1)),  
            float(re.search(rating_pattern, t[2]).group(1)) 
        ),
        dataset
    )

    filtered_items = filter(lambda item: item[1] <= 1000.0, parsed_tuples)
    ...
    sorted_dataset = sorted(mapped_dicts, key=lambda x: x["score"], reverse=True)
